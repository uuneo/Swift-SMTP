...
/// To use the SwiftLog API, please refer to the documentation of ``LoggerMain``.
///
...
/// - The metadata and `logLevel` properties must be implemented so that setting them on a `LoggerMain` does not affect
///   other `LoggerMain`s.
///
...
/// var LoggerMain1 = LoggerMain(label: "first LoggerMain")
/// LoggerMain1.logLevel = .debug
/// LoggerMain1[metadataKey: "only-on"] = "first"
///
/// var LoggerMain2 = LoggerMain1
/// LoggerMain2.logLevel = .error                  // this must not override `LoggerMain1`'s log level
/// LoggerMain2[metadataKey: "only-on"] = "second" // this must not override `LoggerMain1`'s metadata
///
/// XCTAssertEqual(.debug, LoggerMain1.logLevel)
/// XCTAssertEqual(.error, LoggerMain2.logLevel)
/// XCTAssertEqual("first", LoggerMain1[metadataKey: "only-on"])
/// XCTAssertEqual("second", LoggerMain2[metadataKey: "only-on"])
/// ```
///
/// ### Special cases
///
/// In certain special cases, the log level behaving like a value on `LoggerMain` might not be what you want. For example,
/// you might want to set the log level across _all_ `LoggerMain`s to `.debug` when say a signal (eg. `SIGUSR1`) is received
...
///     private static var overrideLogLevel: LoggerMain.Level? = nil
///
///     // this holds the log level if not overridden
///     private var _logLevel: LoggerMain.Level = .info
///
///     // metadata storage
...
///     }
///
///     public func log(level: LoggerMain.Level, message: LoggerMain.Message, metadata: LoggerMain.Metadata?,
...
///     public static func overrideGlobalLogLevel(_ logLevel: LoggerMain.Level) {
///         LogHandlerWithGlobalLogLevelOverride.overrideLock.lock()
///         defer { LogHandlerWithGlobalLogLevelOverride.overrideLock.unlock() }
...
/// level on one `LoggerMain` should not affect the log level of another `LoggerMain` variable.
public protocol LogHandler: _SwiftLogSendableLogHandler {
    ...
    ///
    /// A ``LoggerMain/MetadataProvider`` may add a constant set of metadata,
    /// or use task-local values to pick up contextual metadata and add it to emitted logs.
    var metadataProvider: LoggerMain.MetadataProvider? { get set }

    /// This method is called when a `LogHandler` must emit a log message. There is no need for the `LogHandler` to
    /// check if the `level` is above or below the configured `logLevel` as `LoggerMain` already performed this check and
    ...
    func log(level: LoggerMain.Level,
    ...
    func log(level: LoggerMain.Level, message: LoggerMain.Message, metadata: LoggerMain.Metadata?, file: String, function: String, line: UInt)
    ...
    subscript(metadataKey _: String) -> LoggerMain.Metadata.Value? { get set }

    /// Get or set the entire metadata storage as a dictionary.
    ///
    /// - note: `LogHandler`s must treat logging metadata as a value type. This means that the change in metadata must
    ///         only affect this very `LogHandler`.
    var metadata: LoggerMain.Metadata { get set }
    ...
    ///        `LogHandler`.
    var logLevel: LoggerMain.Level { get set }
}
...
extension LogHandler {
    ...
    @available(*, deprecated, renamed: "log(level:message:metadata:source:file:function:line:)")
    public func log(level: LoggerMain.Level, message: LoggerMain.Message, metadata: LoggerMain.Metadata?, file: String, function: String, line: UInt) {
        self.log(level: level,
                 message: message,
                 metadata: metadata,
                 source: LoggerMain.currentModule(filePath: file),
                 file: file,
                 function: function,
                 line: line)
    }
