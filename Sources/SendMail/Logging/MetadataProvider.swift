...
extension LoggerMain {
    ...
    /// to all logs emitted by a LoggerMain.
    ///
    /// ### Example
    ...
    /// }
    /// let LoggerMain = LoggerMain(label: "example", metadataProvider: metadataProvider)
    /// var baggage = Baggage.topLevel
    /// baggage.traceID = 42
    /// Baggage.withValue(baggage) {
    ///     LoggerMain.info("hello") // automatically includes ["traceID": "42"] metadata
    /// }
    /// ```
    ///
    ...
    public struct MetadataProvider: _SwiftLogSendable {
        /// Provide ``LoggerMain.Metadata`` from current context.
        @usableFromInline
        ...
        /// Invoke the metadata provider and return the generated contextual ``LoggerMain/Metadata``.
...
extension LoggerMain.MetadataProvider {
    ...
    public static func multiplex(_ providers: [LoggerMain.MetadataProvider]) -> LoggerMain.MetadataProvider? {
        assert(!providers.isEmpty, "providers MUST NOT be empty")
        return LoggerMain.MetadataProvider {
